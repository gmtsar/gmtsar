...
#
 if ($#argv != 3 && $#argv != 4) then
  ...
  echo "        filter_wavelength - will sample every 1/2 wavelength (in meters) " 
  echo " "
  exit 1
 endif 
 echo "proj_ra2ll.csh"
...
#
# set the output grid spaccing to be 1/2 the filter wavelength
#
if ($#argv == 4) then
  set filt_wave = $4
  set pix_m = `echo $filt_wave | awk '{print $1/2}'`
  echo "Sampling in geocoordinates with $pix_m meter pixels ..."
else
  set filt = `ls gauss_*`
  if ( $filt != "" ) then
      set pix_m = `ls gauss_* | awk -F_ '{print $2/2}'` # Use 1/2 the filter width
      echo "Sampling in geocoordinates with $pix_m meter pixels ..."
  else
      set pix_m = 60
