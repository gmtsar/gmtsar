...
int write_grdfile(void *API, struct GMT_GRID *T, char *fname, char *prog, char *type, float *data, int verbose) {
	...
	T->data = data;
	if (GMT_Write_Data(API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, fname, T))
		die("cannot create ", fname);
	T->data = NULL;
	return (EXIT_SUCCESS);
}

int read_file_hdr(void *API, char *sim, struct GMT_GRID **IM, char *sre, struct GMT_GRID **RE, unsigned int *xdim,
                  unsigned int *ydim) {
	struct GMT_GRID *I = NULL, *R = NULL;
	if ((I = GMT_Read_Data(API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, sim, NULL)) == NULL)
		die("error reading file", sim);
...
int phasefilt_parse_command_line(char **a, int na, char *USAGE, char *sre, char *sim, float *alp, int *ps, char *amp1, char *amp2,
	...
	for (n = 1; n < na; n++) {
		if (!strcmp(a[n], "-v")) {
			...
			n++;
			if (n == na)
				die(" no option after -imag!\n", "");
			strcpy(sim, a[n]);
			flag[1] = 1;
			if (verbose)
				fprintf(stderr, "imag %s \n", sim);
		}
		else if (!strcmp(a[n], "-alpha")) {
			n++;
			if (n == na)
				die(" no option after -alpha!\n", "");
			*alp = (float)strtod(a[n], NULL);
			...
			n++;
			if (n == na)
				die(" no option after -amp1!\n", "");
			strcpy(amp1, a[n]);
			flag[2] = 1;
			if (verbose)
				fprintf(stderr, "amp1 %s \n", amp1);
		}
		else if (!strcmp(a[n], "-amp2")) {
			n++;
			if (n == na)
				die(" no option after -amp2!\n", "");
			strcpy(amp2, a[n]);
			flag[3] = 1;
			if (verbose)
				fprintf(stderr, "amp2 %s \n", amp2);
		}
		else if (!strcmp(a[n], "-psize")) {
			n++;
			if (n == na)
				die(" no option after -psize!\n", "");
			*ps = atoi(a[n]);
			if (verbose)
			...
			*dflag = 1;
			if (verbose)
				fprintf(stderr, "calculate difference \n");
		}
	...
	*/
	...
	if (flag[0] == 0)
		die("real file not set", "");
	if (flag[1] == 0)
		die("imag file not set", "");
...
char *USAGE = "phasefilt [GMTSAR] - Apply adaptive non-linear phase filter\n\n"
              "\n USAGE:\nphasefilt -imag imag.grd -real real.grd [-alpha alpha][-psize "
              "size][-amp1 amp1.grd -amp2 amp2.grd][-diff][-v]\n"
              " applies Goldstein adaptive filter to phase [output: filtphase.grd]\n"
              " or applies modified Goldstein adaptive filter to phase [output: "
              "filtphase.grd, corrfilt.grd]\n"
              "-imag [required] GMT format file of imaginary component\n"
              "-real [required] GMT format file of real component\n"
              "-alpha 	exponent for filter - usually between 0.0 and 1.5 (0.0 should "
              "not filter).\n"
              "		default: 0.5	[Goldstein filter] (anything above 1.0 may be "
              "excessive)\n"
              "		alpha < 0 will set alpha = (1 - coherence) [modified "
              "Goldstein]\n"
              "-psize 	patch size for filtering. Must be power of two.\n"
              "		default: 32\n"
              "-amp1 		GMT format file of amplitude image of image 1. Needed "
              "(and applies) modified filter.\n"
              "-amp2 		GMT format file of amplitude image of image 2. Needed "
              "(and applies) modified filter.\n"
              "-diff 		Calculate difference between input phase and output "
              "phase.\n"
              "-complex_out	Write out filtered real and imaginary "
              "(filtphase_real.grd and filtphase_imag.grd)\n"
              "-v 		Verbose.\n"
              "-debug 	Debug.\n"
              "\n"
              "example:\n"
...
int main(int argc, char **argv) {
	...
	if ((T = GMT_Create_Data(API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, IM->header->wesn, IM->header->inc,
	                         IM->header->registration, 0, NULL)) == NULL)
		die("could not allocate grid header", "");

	if ((data = malloc(T->header->nm * sizeof(struct FCOMPLEX))) == NULL)
		die("error allocating memory", "");
	if ((fdata = malloc(T->header->nm * sizeof(struct FCOMPLEX))) == NULL)
		die("error allocating memory", "");
	if ((patch0 = malloc(nxp * nyp * sizeof(struct FCOMPLEX))) == NULL)
		die("error allocating memory", "");
	if ((patch1 = malloc(nxp * nyp * sizeof(struct FCOMPLEX))) == NULL)
		die("error allocating memory", "");
	if ((amp = malloc(T->header->nm * sizeof(float))) == NULL)
		die("error allocating memory", "");
	if ((wgt = malloc(nxp * nyp * sizeof(float))) == NULL)
		die("error allocating memory", "");
	if ((outphase = malloc(xdim * (ydim + nyp) * sizeof(float))) == NULL)
		die("error allocating memory", "");

	/* array for difference */
	if (dflag) {
		if ((diff = malloc(T->header->nm * sizeof(float))) == NULL)
			die("error allocating memory", "");
	}

	/* array for writing filtered real and imag  */
	if (comflag) {
		if ((ftmp = malloc(T->header->nm * sizeof(float))) == NULL)
			die("error allocating memory", "");
	}

	for (i = 0; i < T->header->nm; i++)
		outphase[i] = fdata[i].r = fdata[i].i = 0.0f;
	...
	if (dflag) {
		for (i = 0; i < T->header->nm; i++)
			diff[i] = atan2f(data[i].i, data[i].r) - outphase[i];
