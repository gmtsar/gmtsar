...
| Case | csh | py | Speedup |
|---|---:|---:|---:|
| `NISAR_Ethiopia` | 543s | 200s | **2.71×** |
| `RS2_SLC_Hawaii` | 195s | 108s | **1.81×** |
| `ALOS_SLC_L1.1` | 448s | 357s | 1.25× |
| `TSX_SLC_Hawaii` | 852s | 693s | 1.23× |
| `CSK_RAW_Hawaii` | 822s | 719s | 1.14× |
| `S1_Ridgecrest_EQ` | 9472s | 8328s | 1.14× |
| `ALOS2_SCAN_SSAF` | 9080s | 8079s | 1.12× |
| ... 14 more cases ... | | | 0.91×–1.06× |

Aggregate (wall-time weighted across all 21 cases): **1.06×**. Full per-case table, per-binary cost breakdown, and hardware/software snapshot: `docs/perf_snapshots/perf_snapshot_2026-07-13T10-26-01Z_566566e_full.md`.

Example comparison (`NISAR_Ethiopia`, the fastest case): Python output (left) vs. legacy csh (right), same interferogram, pixel-for-pixel — generated by `tools/py_vs_csh_figure.py` from real sweep output, not a mockup.
