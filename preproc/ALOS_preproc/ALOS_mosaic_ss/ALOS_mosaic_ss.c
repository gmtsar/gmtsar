...
char *USAGE = " ALOS_mosaic_ss prmfile1 grd1 prmfile2 grd2 prmfile3 grd3 prmfile4 grd4 prmfile5 grd5 mosaic dec \n\n" 
" USAGE:  \n"
...
int main(int argc, char **argv) {
  ...
  if ((API = GMT_Create_Session (argv[0], 2, 0, NULL)) == NULL) return EXIT_FAILURE;
  ...
  vel[0] = prm.vel;
  fclose(prmfile);
  if ((grid1 = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, argv[2], NULL)) == NULL) return EXIT_FAILURE;
  ...
  if ((num_rng[0] != round(grid1->header->nx * grid1->header->inc[GMT_X])) || (num_azi[0] != round(grid1->header->ny * grid1->header->inc[GMT_Y]))) { 
    die("\nPRM file and GMT grid don't match!\n",argv[2]);
  }
  if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, argv[2], grid1) == NULL) return EXIT_FAILURE;
  ...
  vel[1] = prm.vel;
  fclose(prmfile);
  if ((grid2 = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, argv[4], NULL)) == NULL) return EXIT_FAILURE;
  /* check if the dimension of the grid and PRM file match */
  ...
  if ((num_rng[1] != round(grid2->header->nx * grid2->header->inc[GMT_X]) ) || (num_azi[1] != round(grid2->header->ny * grid2->header->inc[GMT_Y]))) {      
    die("\nPRM file and GMT grid don't match!\n",argv[4]);
  }
  if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, argv[4], grid2) == NULL) return EXIT_FAILURE;
  ...
  vel[2] = prm.vel;
  fclose(prmfile);
  if ((grid3 = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, argv[6], NULL)) == NULL) return EXIT_FAILURE;
  /* check if the dimension of the grid and PRM file match */
  ...
  if ((num_rng[2] != round(grid3->header->nx*grid3->header->inc[GMT_X])) || (num_azi[2] != round(grid3->header->ny*grid3->header->inc[GMT_Y] ))) {           
    die("\nPRM file and GMT grid don't match!\n",argv[6]);
  }
  if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, argv[6], grid3) == NULL) return EXIT_FAILURE;
  ...
  vel[3] = prm.vel;
  fclose(prmfile);
  if ((grid4 = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, argv[8], NULL)) == NULL) return EXIT_FAILURE;
  /* check if the dimension of the grid and PRM file match */
  ...
  if ((num_rng[3] != round(grid4->header->nx*grid4->header->inc[GMT_X])) || (num_azi[3] != round(grid4->header->ny*grid4->header->inc[GMT_Y] ))) {
    die("\nPRM file and GMT grid don't match!\n",argv[8]);
  }
  if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, argv[8], grid4) == NULL) return EXIT_FAILURE;
  ...
  vel[4] = prm.vel;
  fclose(prmfile);
  if ((grid5 = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_HEADER_ONLY, NULL, argv[10], NULL)) == NULL) return EXIT_FAILURE;
  /* check if the dimension of the grid and PRM file match */
  ...
  if ((num_rng[4] != round(grid5->header->nx*grid5->header->inc[GMT_X])) || (num_azi[4] != round(grid5->header->ny * grid5->header->inc[GMT_Y]))) {
    die("\nPRM file and GMT grid don't match!\n",argv[10]);
  }
  if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_DATA_ONLY, NULL, argv[10], grid5) == NULL) return EXIT_FAILURE;
  ...
  if ((grid6 = GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, wesn, inc, \
                GMT_GRID_PIXEL_REG, 0, NULL)) == NULL) die("could not allocate grid header","");

  if (GMT_Set_Comment (API, GMT_IS_GRID, GMT_COMMENT_IS_REMARK, "mosaic", grid6)) return EXIT_FAILURE;

  /* fill the empty grid with nan */
  for (node6 = 0; node6 < grid6->header->size; node6++) grid6->data[node6] = NAN;
  ...
  for (range = ra[0][0]; range < range_boundary1; range ++) { 
    for (azimuth = ra[0][2]; azimuth < ra[0][3]; azimuth++) {
      ...
      row = round((azimuth*dec+clock_start_rel[0]*vel[0])/(vel[0]/prf[0])/grid1->header->inc[GMT_Y]);
  ...
  for (range = range_boundary1; range < range_boundary2; range ++) {
    for (azimuth = ra[1][2]; azimuth < ra[1][3]; azimuth++) {
      ...
      row = round((azimuth*dec+clock_start_rel[1]*vel[1])/(vel[1]/prf[1])/grid2->header->inc[GMT_Y]);
...
/* put in subswath 3 */
  ...
  for (range = range_boundary2; range < range_boundary3; range ++) {
    for (azimuth = ra[2][2]; azimuth < ra[2][3]; azimuth++) {
      ...
      row = round((azimuth*dec+clock_start_rel[2]*vel[2])/(vel[2]/prf[2])/grid3->header->inc[GMT_Y]);
...
/* put in subswath 4 */
  ...
  for (range = range_boundary3; range < range_boundary4; range ++) {
    for (azimuth = ra[3][2]; azimuth < ra[3][3]; azimuth++) {
      ...
      row = round((azimuth*dec+clock_start_rel[3]*vel[3])/(vel[3]/prf[3])/grid4->header->inc[GMT_Y]);
...
/* put in subswath 5 */
  ...
  for (range = range_boundary4; range < range_boundary5; range ++) {
    for (azimuth = ra[4][2]; azimuth < ra[4][3]; azimuth++) {
      ...
      row = round((azimuth*dec+clock_start_rel[4]*vel[4])/(vel[4]/prf[4])/grid5->header->inc[GMT_Y]);
  ...
  /* output and clean up */
  if (GMT_Write_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_GRID_ALL, NULL, argv[11], grid6)) return EXIT_FAILURE;
