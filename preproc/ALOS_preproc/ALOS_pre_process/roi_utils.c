...
int write_roi(char *imagery, FILE *ldrfile, struct PRM prm, struct ALOS_ORB orb, char *date) {
	...
	if ((datafile = fopen(imagery, "r")) == NULL)
		die("error opening ", imagery);
	...
	rewind(ldrfile);
	(void)fread(&sb, sizeof(struct sarleader_binary), 1, ldrfile);
	...
	if ((rsc = fopen(foutname, "w")) == NULL)
		die("error opening tmp_raw.rsc", "");
...
/*--------------------------------------------------------------------------------------------------------------*/
int prm2roi(struct PRM prm, double *start_time, double *starting_range, int *first_sample, double *prf, double *chirp, int *width,
            int *xmin, int *xmax) {
	*prf = prm.prf;
...
/*--------------------------------------------------------------------------------------------------------------*/
int get_utc(double start_time, struct SAR_info sar, double *first_line_utc, double *last_line_utc, double *center_utc, double pri,
            int file_length) {
...
int write_roi_orbit(struct ALOS_ORB orb, char *date) {
	...
	if ((orbit_rsc = fopen(fname, "w")) == NULL)
