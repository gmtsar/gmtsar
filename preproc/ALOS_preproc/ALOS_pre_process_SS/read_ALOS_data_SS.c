...
clock_start clock_stop        - this has not year for better precision
DDD.DDDDDDDDD
*/
...
int64_t read_ALOS_data_SS(FILE *imagefile, FILE *outfile, struct PRM *prm, int64_t *byte_offset, int *nsub, int *burst_skip,
                       int *num_burst) {
	...
	int totrecl;          /* total record length	   11200 bytes = line_prefix_size +
	                         record_length */
	...
	/* get the sarting burst number and the PRF information for all the bursts and
	 * rewind the file */
	for (k = 0; k < 5; k++)
		totburst = totburst + n_burst[k];
	for (j = 0; j < totburst; j++) {
		if (fread((void *)&sdr, sizeof(struct sardata_info), 1, imagefile) != 1)
			break;
	...
	for (k = 0; k < 5; k++)
		if (PRF[k] > 0.)
			dt = dt + (double)n_burst[k] / (0.001 * PRF[k]);
	...
	if (verbose)
		fprintf(stderr, " totburst start_busrt# burstcycle_time %d %d %f \n", totburst, kswath0, dt);

	/* set the total number of lines output based on the number of patches
	 * requested or default 1000 */
	ntot = *num_burst * (n_burst[*nsub] + ngap);
	if (verbose)
		fprintf(stderr, " dt, tgap, n_burst, ngap ,ntot %f %f %d %d %d %d \n", dt, tgap, n_burst[*nsub], ngap, prm->num_valid_az,
		        ntot);

	/* allocate the memory for data */
	if ((data = (char *)malloc(record_length0)) == NULL)
		die("couldn't allocate memory for input indata.\n", "");
	if ((shift_data = (char *)malloc(record_length0)) == NULL)
		die("couldn't allocate memory for input indata.\n", "");
	if ((gap_data = (char *)malloc(record_length0)) == NULL)
		die("couldn't allocate memory for input indata.\n", "");

	/* seek to beginning of next burst of sub swath nsub, read the line header,
	 * reset the parameters, and set the counters */
	skip_swath = ((*nsub + 5) - kswath0) % 5;
	skiprecl = 0;
	for (k = kswath0; k < kswath0 + skip_swath; k++)
		skiprecl = skiprecl + n_burst[k % 5];
	skiprecl = skiprecl + *burst_skip * totburst;
	if (verbose)
		fprintf(stderr, " skip_swath skiprecl %d %d \n", skip_swath, skiprecl);
	...
	/* get the time interval of a burst at the new skip location and rewind the
	 * file */
	dt = 0.;
	for (k = 0; k < 5; k++)
		if (PRF[k] > 0.)
	...
	*byte_offset = ftell(imagefile);
	if (verbose)
		fprintf(stderr, " n skiprecl byte_offset %d %d %ld \n", n, skiprecl, *byte_offset);
	...
	while ((fread((void *)&sdr, sizeof(struct sardata_info), 1, imagefile)) == 1) {
		...
		if (sdr.n_data_pixels == n_data_burst[*nsub]) {
			...
			/* check shift to see if it varies from beginning or from command line
			 * value */
			...
			if ((verbose) && (n / 2000.0 == n / 2000)) {
				fprintf(stderr, " Working on line %d prf %f record length %d slant_range %d \n", sdr.sequence_number,
				        0.001 * sdr.PRF, record_length1, sdr.slant_range);
			}

			/* read data (and trailing bytes) */
			if (fread((char *)data, record_length1, (size_t)1, imagefile) != 1)
				break;
			...
			/* Shimada says the first 13 lines are bad.  I only saw 11 bad. shift
			 * these lines outside the window */
			if (kburst < 12)
				ishift = data_length;
			...
			/* if kburst it equal to the length of this burst then write the
			 * appropriate number of zero lines */
			if (kburst == n_burst[*nsub]) {
				ttot = ttot + dt;
				if (verbose)
					fprintf(stderr, " %d %d %f %f %d %f \n", *nsub + 1, kburst, dt, tgap, ngap, ttot);
				kburst = 0;
				...
				for (j = 0; j < ngap; j++) {
					...
					for (k = 0; k < record_length0; k++)
						gap_data[k] = NULL_DATA + znew % 2;
...
int fill_shift_data(int shift, int ishift, int data_length, int line_suffix_size, int record_length1, char *data,
	...
	if (shift > 0) {
		for (k = 0; k < ishift; k++)
			shift_data[k] = NULL_DATA + znew % 2;
		for (k = 0; k < data_length - ishift; k++)
			shift_data[k + ishift] = data[k];
		for (k = data_length - ishift; k < record_length1 - ishift; k++)
			shift_data[k + ishift] = NULL_DATA + znew % 2;
	}
