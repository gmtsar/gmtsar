...
int write_slc_hdf5(hid_t input, FILE *slc, double SLC_factor) {
	...
	dset = H5Dopen(group, "IMG", H5P_DEFAULT);
...
int pop_led_hdf5(hid_t input, state_vector *sv) {
    ...
    printf("Reading %d state vectors ... \n", count);
...
int pop_prm_hdf5(struct PRM *prm, hid_t input, char *file_name) {
	...
	if (strcmp(rec, "SPOTLIGHT") == 0) {
		hdf5_read(tmp_d, input, "/S01", "IMG", "Line Time Interval", 'd');
		prm->prf = 1.0 / tmp_d[0];
	}
	...
	if (strcmp(rec, "RAW_B") == 0) {
		...
		// SLC
		hdf5_read(tmp_d, input, "/S01", "IMG", "Zero Doppler Range First Time", 'd');
		prm->near_range = tmp_d[0] * c_speed / 2;

		hdf5_read(tmp_c, input, "/", "", "Reference UTC", 'c');
		hdf5_read(tmp_d, input, "/S01", "IMG", "Zero Doppler Azimuth First Time", 'd');
		cat_nums(date, tmp_c);
		str_date2JD(tmp_c, date);
