...
int main(int argc, char **argv) {
	...
	if (err_code == e_err_none) {
		...
		if (err_code == e_err_none) {
			...
			printf("envisat_dump_header: non-fatal error in epr_read_record "
			       "MDS1_SQ_ADS\n");
	...
	if (err_code == e_err_none) {
		...
		if (err_code == e_err_none) {
			...
			printf("envisat_dump_header: error in epr_read_record "
		...
		printf("envisat_dump_header: error in epr_get_dataset_id "
		       "MAIN_PROCESSING_PARAMS_ADS\n");
		printf("could not read this record.  may not be a problem.\n");
		epr_clear_err();
	}
