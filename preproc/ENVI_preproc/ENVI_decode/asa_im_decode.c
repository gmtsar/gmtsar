...
v1.05, Mar 25, 2004, Sean M. Buckley, now fills missing lines with zeroes in
float mode and 0.*127.5+127.5 + .5 = 128 for byte mode v1.1, 17 Feb 2005,
Zhenhong Li:
        1. This program can run on a little endian machine as well as a big
endian machine!
        2. On Linux, compiled with command gcc -O2 asa_im_decode.c -o
asa_im_decode
        3. On Unix, compiled with command gcc -O2 asa_im_decode.c -o
asa_im_decode v1.1.1, 8 Nov 2005, Vikas Gudipati, now runs correctly on 64-bit
compilers.
...
int main(int argc, char *argv[]) {
	...
	unsigned short TxPulseLengthCodeword = 0;
	unsigned short priCodeword = 0;
	unsigned short priCodewordOld = 0;
	unsigned short priCodewordOldOld = 0;
	unsigned short windowStartTimeCodeword = 0;
	unsigned short windowStartTimeCodeword0 = 0;
	unsigned short windowStartTimeCodewordOld = 0;
	unsigned short windowStartTimeCodewordOldOld = 0;
	...
	sscanf(argv[numFiles + 4], "%lf", &near_range);

	/*//debug
	numFiles = 1;
	//sscanf(
	"D:\\data\\scign\\ASAR_RAW\\09786-2925-20040113\\ASA_IM__0CNPDK20040113_180720_000000152023_00213_09786_1579.N1",
	"%s", insFileName ); sscanf(
	"D:\\data\\scign\\ASARAux\\ASA_INS_AXVIEC20031209_113421_20030211_000000_20041231_000000",
	"%s", insFileName ); sscanf( "D:\\temp\\tmp_IMAGERY.raw", "%s", outFileName );
	sscanf( "1", "%d", &outType );
	...
	*/
	...
	// sphIns = readSphAux( sphPtr, printInsSphIfZero, mphIns );	/* extract
	// information from SPH */
	(void)readSphAux(sphPtr, printInsSphIfZero, mphIns); /* extract information from SPH */
	free(sphPtr);
	...
	if ( (fread( gadsPtr, sizeof( char ), sizeof( insGads ), insFilePtr ) ) !=
	sizeof( insGads ) ){ printf( "sizeof( insGads ): %d\n", sizeof( insGads ) );
	...
	*/
	...
	if (printInsGadsIfZero == 0)
		printInsGads(insGads);
	...
	for (ii = 0; ii < numFiles; ii++) {
		...
		for (i = 0; i < sph.dsd[0].numDsr; i++) {
			...
			*/
			...
			if (printImMdsrIfZero == 0) {
				...
				/* onBoardTimeFractionalSeconds = (double)
				 * ((double)onBoardTimeFractionalSecondsInt/65536.); */
				printf("%6d %2u %2x %8d %5d %d %d %d %d %d %d %d %4d %5d %5d %5d %2d "
				       "%2d %1d %1d %2u %4d %2d %3d %3d %5d\n",
				       i + 1, dataFieldHeaderLength, modeID, onBoardTimeIntegerSeconds, onBoardTimeFractionalSecondsInt,
				       modePacketCount, antennaBeamSetNumber, compressionRatio, echoFlag, noiseFlag, calFlag, calType,
				       cyclePacketCount, priCodeword, windowStartTimeCodeword, windowLengthCodeword, upConverterLevel,
				       downConverterLevel, TxPolarization, RxPolarization, calibrationRowNumber, TxPulseLengthCodeword,
				       beamAdjDeltaCodeword, chirpPulseBandwidthCodeword, auxTxMonitorLevel, resamplingFactor);
			}
			...
			if ((modePacketCount == modePacketCountOld + 1) || (firstTimeEqualsZero == 0)) {
				...
				if ((echoFlag == 1) && (noiseFlag == 0) && (calFlag == 0)) {
					...
					if (firstTimeEqualsZero == 0) {
						...
						/*  compute windowStartTimeCodeword0 from the near_range Jan 21 2011
						 * by Xiaopeng */
						if (fabs(near_range) < 1e-9)
							windowStartTimeCodeword0 = windowStartTimeCodeword;
						else {
							pri = priCodeword / insGads.sampRate;
							swst0 = near_range * 2 / c - pri * insGads.timelineIm.rValues[antennaBeamSetNumber - 1];
							windowStartTimeCodeword0 = (int)floor(swst0 * insGads.sampRate + 0.5);
						}
						/* end of modification: compute windowStartTimeCodeword0 from the
						 * near_range */

						if (windowStartTimeCodeword0 == 0)
							windowStartTimeCodeword0 = windowStartTimeCodeword;
						else if (windowStartTimeCodeword0 != windowStartTimeCodeword)
							printf("Line %5d : windowStartTimeCodeword %5d : shifting this "
					...
					/* check a few things  - still need to check TxPulseLength,
					 * chirpPulseBandwidthCodeword, beamAdjDeltaCodeword */

					if ((i != 0) && (priCodeword != priCodewordOld)) {
						printf("Line %5d : priCodeword changes from %5d to %5d : no action "
						       "taken\n",
						       i + 1, priCodewordOld, priCodeword);
					}

					if (windowStartTimeCodeword != windowStartTimeCodewordOld) {
						printf("Line %5d : windowStartTimeCodeword changes from %5d to %5d "
						       ": shifting this and subsequent data to %5d\n",
					...
					for (j = 0; j < (mdsrIspLength + 1 - 30) / 64; j++) {
						...
						for (k = 0; k < 63; k++) {
							...
							/* if ( i == 0 ) {
							   printf( "k,sample,blockId,i_in,q_in,i_out,q_out: %2d %4d %3d
							%2d %2d %15f %15f\n", k, 63*j+k,  mdsrBlockId[ j ], ((mdsrCheck[ k
							] >> 4) & 15), ( mdsrCheck[ k ] & 15), mdsrLine[ 2*k ], mdsrLine[
							2*k+1 ] );
							} */
						}
					}
					...
					for (k = 0; k < ((mdsrIspLength + 1 - 30) % 64) - 1; k++) {
						...
						mdsrLine[2 * 63 * j + 2 * k + 1] = LUTq[256 * (15 - (mdsrCheck[k] & 15)) + mdsrBlockId[j]];
						/* if ( i == 0 ) {
						   printf( "k,sample,blockId,i_in,q_in,i_out,q_out: %2d %4d %3d %2d
						%2d %15f %15f\n", k, 63*j+k,  mdsrBlockId[ j ], ((mdsrCheck[ k ] >>
						4) & 15), ( mdsrCheck[ k ] & 15), mdsrLine[ 2*k ], mdsrLine[ 2*k+1 ]
						);
						} */
					}
				...
				modePacketCount, modePacketCountOld ); fseek( imFilePtr,
				mdsrIspLength+1-30, SEEK_CUR ); bytesRead = bytesRead +
				mdsrIspLength+1-30;
				*/
		...
		/*
		      ...
		      printf( "%s%d\n",      "swathNum:                             ",
		   insGads.timelineIm.swathNums[ antennaBeamSetNumber-1 ] ); printf(
		   "%s%d\n",      "mValue:                               ",
		   insGads.timelineIm.mValues[ antennaBeamSetNumber-1 ] ); printf( "%s%d\n",
		   "rValue:                               ", insGads.timelineIm.rValues[
		   antennaBeamSetNumber-1 ] ); printf( "%s%d\n",      "gValue: ",
		   insGads.timelineIm.gValues[ antennaBeamSetNumber-1 ] ); printf(
		   "%s%.9g\n",    "(rank*pri+windowStartTime)*c/2 (m):   ",
		   (insGads.timelineIm.rValues[ antennaBeamSetNumber-1
		   ]*pri+windowStartTime)*c/2. ); printf( "%s%.9g\n",
		   "(last)windowStartTime*c/2 (m):        ", windowStartTime*c/2. ); printf(
		   "%s%.9g\n",    "windowLength*c/2 (m):                 ",
		   windowLength*c/2. ); printf( "%s%.9g\n",    "rangeGateBias*c/2 (m): ",
		   insGads.rangeGateBias*c/2. );

		      printf( "\nOutput information:\n\n" );
		      printf( "%s%d\n",      "number of output samples:             ",
		   outSamples ); printf( "%s%d\n",      "number of output lines: ", outLines
		   ); printf( "%s%.9g\n",    "chirp pulse bandwidth (Hz):           ",
		   chirpPulseBandwidth ); printf( "%s%.9g\n",    "prf (Hz): ", 1./pri );
		      printf( "%s%.9g\n",    "range sampling frequency (Hz):        ",
		   insGads.sampRate ); printf( "%s%.9g\n",    "range sample spacing (m): ",
		   c/(2.*insGads.sampRate)); printf( "%s%.9g\n",    "chirp slope (Hz/s): ",
		   chirpPulseBandwidth/TxPulseLength ); printf( "%s%.9g\n",    "pulse length
		   (s):                     ", TxPulseLength ); printf( "%s%.9g\n", "radar
		   frequency (Hz):                 ", insGads.radarFrequency ); printf(
		   "%s%.9g\n",    "wavelength (m):                       ",
		   c/insGads.radarFrequency ); printf( "%s%.9g\n",    "starting range (m):
		   ", (insGads.timelineIm.rValues[ antennaBeamSetNumber-1
		   ]*pri+windowStartTime)*c/2. ); printf( "\n" );
		*/
	...
	printf("%s%.9g\n", "starting range (m):                   ",
	       (insGads.timelineIm.rValues[antennaBeamSetNumber - 1] * pri + windowStartTime) * c / 2.);
...
void printInsGads(const struct insGadsStruct insGads) {
	...
	for (i = 0; i < 4096; i++)
		printf("%s%4d%s%15f %15f %15f\n", "fbaq4LutI,Q,NoAdc[ ", i, " ]:          ", insGads.fbaq4LutI[i], insGads.fbaq4LutQ[i],
		       insGads.fbaq4NoAdc[i]);
	printf(".\n");
	printf(".\n");
	printf(".\n");
 ...
 **
 ** Purpose: Convert the bytes of struct insGadsStruct for a little endian order
machine
 **
