...
int pop_led(tree *xml_tree, state_vector *sv) {
	...
	while (1) {
		...
		search_tree(xml_tree,
		            "/product/sourceAttributes/orbitAndAttitude/orbitInformation/"
		            "stateVector/yPosition/",
		            tmp_c, 1, 5, i);
		sv[i - 1].y = str2double(tmp_c);
		search_tree(xml_tree,
		            "/product/sourceAttributes/orbitAndAttitude/orbitInformation/"
		            "stateVector/zPosition/",
		            tmp_c, 1, 5, i);
		sv[i - 1].z = str2double(tmp_c);
		search_tree(xml_tree,
		            "/product/sourceAttributes/orbitAndAttitude/orbitInformation/"
		            "stateVector/xVelocity/",
		            tmp_c, 1, 5, i);
		sv[i - 1].vx = str2double(tmp_c);
		search_tree(xml_tree,
		            "/product/sourceAttributes/orbitAndAttitude/orbitInformation/"
		            "stateVector/yVelocity/",
		            tmp_c, 1, 5, i);
		sv[i - 1].vy = str2double(tmp_c);
		search_tree(xml_tree,
		            "/product/sourceAttributes/orbitAndAttitude/orbitInformation/"
		            "stateVector/zVelocity/",
		            tmp_c, 1, 5, i);
...
int pop_prm(struct PRM *prm, tree *xml_tree, char *file_name) {
	...
	prm->SC_identity = 9;                        /* (1)-ERS1 (2)-ERS2 (3)-Radarsat (4)-Envisat (5)-ALOS
