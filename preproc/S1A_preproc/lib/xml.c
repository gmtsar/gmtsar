...
/***************************************************************************
    ...
    str_out  - pointer to the output string(everything will be output as string). 
    str      - is the string used for searching in the shape of /lev1_name/lev2_name/.../levN_name/
               Note all the names should be seperated by '/', including the start and end with '/'. 
    type     - choice of return where
               1 string to str_out
               2 converted date to str_out with the format yyyyddd.frac_of_day
               3 values in the names, for example <orbitLists 'count = 31'> will return 31

    For names occurred in multiple locations, use loc and num to return the one at correct location. 
    loc      - the term to be repeatedly found,
    num      - index of array.
               For example, to search /product/OrbitInformation/OrbitList/orbit/position/x/
               and there are multiple state vectors under Orbitlist, set loc = 4, the 4th value of x
