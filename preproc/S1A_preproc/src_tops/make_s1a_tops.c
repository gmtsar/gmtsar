...
int dramp_dmod(struct tree *, int, fcomplex *, int , int, int, struct GMT_GRID *, struct GMT_GRID *, int, int);
int shift_write_slc(void *, struct PRM *, struct tree *, burst_bounds *, int, TIFF *, FILE *, FILE *, FILE *, char *, char *);
...
int dramp_dmod (struct tree *xml_tree, int nb, fcomplex *cramp, int lpb, int width, int al_start, struct GMT_GRID *R, struct GMT_GRID *A, int bshift, int imode){
...
int shift_write_slc(void *API,struct PRM *prm,struct tree *xml_tree,struct burst_bounds *bb,int imode,TIFF *tif,FILE *slcl,FILE *slcc,FILE *slch,char *dr_table,char *da_table) {
...
*/
    ...
    for (ii=0;ii<lpb;ii++){
		...
		cbrst[k].i = 0;
                ...
                ras[1] = (double)ii+A->data[(int)floor(jj/incx+0.5)+A->header->nx*(int)floor((al_start+ii)/incy+0.5)]  - (double)bshift;
