...
int shift_write_slcs(void *, struct PRM *, struct tree *, burst_bounds *, TIFF *, FILE *, FILE *, FILE *, int, double, double,
...
char *USAGE = "\nUsage: make_s1a_tops_6par xml_file tiff_file output mode rng_shift "
              "azi_shift stretch_a a_stretch_a stretch_r a_stretch_r\n"
              ...
              "\nExample: make_s1a_tops_6par s1a-s1-slc-vv-20140807.xml "
              "s1a-s1-slc-vv-20140807.tiff S1A20140807 1 0.4596 .9109 2.18065e-06 "
              "-3.63255e-06 1.37343e-05 -3.13352e-05 \n"
...
int main(int argc, char **argv) {
	...
	shift_write_slcs(API, &prm, xml_tree, bb, TIFF_FILE, OUTPUT_SLCL, OUTPUT_SLCC, OUTPUT_SLCH, imode, rng, azi, stretch_a,
	                 a_stretch_a, stretch_r, a_stretch_r);
...
int shift_write_slcs(void *API, struct PRM *prm, tree *xml_tree, burst_bounds *bb, TIFF *tif, FILE *slcl, FILE *slcc, FILE *slch,
...
int rng_interp(fcomplex *cbrst, fcomplex *ctmp, int cl, int lstart, int lend, int width, double rng, double stretch_r,
               double a_stretch_r) {
	...
	for (ii = lstart; ii <= lend; ii++) {
		...
		for (jj = 0; jj < width; jj++) {
			...
			j = (int)ra;
			// make sure the point is safe for interpolation, use original input if
			// can not be interpolated
			if (j - ns2 < 0 || j + ns2 + 1 >= width)
				continue;
